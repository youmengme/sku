 ...
 * @returns
 */
function cloneMatrix(o) {
  let res = []
  o.forEach((ele) => {
    res.push(ele.slice ? ele.slice() : ele)
  })
  return res
}
...
export class PathFinder {
   ...
   */
  init() {
    this.light = cloneMatrix(this.maps)
  ...
  _check(isAdd) {
    ...
    for (let i = 0; i < light.length; i++) {
      const li = light[i]
      const selectedPrima = this.getSelectedPrima(i)
      for (let j = 0; j < li.length; j++) {
        if (li[j] !== 2) {
          // 如果是加一个条件，只在是light值为1的点进行选择
          if (isAdd) {
            if (li[j]) {
              light[i][j] = this.checkItem(maps[i][j], selectedPrima)
              this.count++
            }
          } else {
            light[i][j] = this.checkItem(maps[i][j], selectedPrima)
  ...
  /**
   * 检查是否可选内容
   * @param { Number } item 当前规格质数
   * @param { Number } selectedPrima 已选规格的质数
   * @returns
   */
  checkItem(item, selectedPrima) {
    // 拿到可以选择的 SKU 内容集合
    const openWay = this.openWay
    const val = item * selectedPrima
    // 拿到已经选中规格集合*此规格集合值
    // 可选 SKU 集合反除，查询是否可选
   ...
   */
  add(point) {
    if (!Array.isArray(point)) point = this._way[point]
    const val = this.maps[point[0]][point[1]]

    // 检查是否可选中
    if (!this.light[point[0]][point[1]]) {
      throw new Error(
        'this point [' + point + '] is unavailable, place choose an other'
      )
    }
